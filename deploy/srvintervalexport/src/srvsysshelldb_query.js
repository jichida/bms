...
const export_history = require('./lib/export_history_shell_query');
...
const cron_0 = (startDay,endDay,callbackfn)=>{
  let curtime = moment().format('YYYY-MM-DD HH:mm:ss');
  winston.getlog().info(`零点开始执行-->${curtime}`);
  let fnsz = [];

  export_history(startDay,endDay,(exportdir)=>{
...
const start_cron0 = (startDay,endDay,callbackfnall)=>{
  cron_0(startDay,endDay,(err,result)=>{
