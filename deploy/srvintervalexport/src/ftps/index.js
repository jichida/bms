...
const ftps = new FTPS({
  ...
  retries: 0, // Optional, defaults to 1 (1 = no retries, 0 = unlimited retries)
...
const sftptosrv = (localdir,localfilename,callback)=>{
  ...
  winston.getlog().info(`本地目录${localdir},文件${localfilename},远程目录: data/${localfilename}`);
  ftps.put(`${localdir}/${localfilename}`, `data/${localfilename}`).exec((err, res)=> {
    winston.getlog().info(`上传文件到data目录:${localdir}/${localfilename},${!!err}`);
    if(!err){
      winston.getlog().info(`上传文件错误->${JSON.stringify(err)}`);
      console.log(err);
    }
    callback(err,res);
  });
